  ...
  describe("tfxInit", () => {
    it("should throw an error if more than two arguments are passed", () => {
      ...
      try {
        ...
        assert.deepEqual(
          err,
          `${constants.ansiRed}tfx init expects only a single arcument (directory path). To get a list of valid commands, run \`tfx --help\`.${constants.ansiDefaultForeground}`,
