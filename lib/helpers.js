...
const { containsKeys, typeCheck, keyCheck } = require("./utils");
...
const helpers = {
  ...
  checkResourceTests: function (resources) {
    ...
    resources.forEach((test) => {
      ...
      typeCheck(`Value fields for test objects must be type`, "object", test.values);
    });
  },
  ...
  eachKey: function (obj, callback) {
    typeCheck(`eachKey expects the the first argument to be type`, "object", obj)
    typeCheck(`eachKey expects callback to be type`, "Function", callback)
