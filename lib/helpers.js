...
/**
 * restore stringified json to tfx list
 * @param {object} json data with escaped string values replaced
 * @param {Array<string>} stringList list of values to insert
 * @returns {object} object with json restored
 */
function restoreStringifiedJson(json, stringList) {
  /**
   * match replacement json
   * @param {string} str
   * @returns {boolean} true if does match
   */
  function jsonReplacementMatch(str) {
    return str.match(/^__json_value_\d\d?$/) !== null;
  }

  /**
   * replace array instance text
   * @param {Array} arr array
   */
  function searchArray(arr) {
    for (let i = 0; i < arr.length; i++) {
      if (isString(arr[i])) {
        if (jsonReplacementMatch(arr[i])) {
          let index = arr[i].replace(/\D/g, "");
          arr[i] = stringList[index];
        }
      } else if (isArray(arr[i])) {
        searchArray(arr[i]);
      } else if (getType(arr[i]) === "object") {
        restoreStringifiedJson(arr[i], stringList);
      }
    }
  }
  ...
  eachKey(json, (key) => {
    if (isString(json[key])) {
      // if is string replace text match
      if (jsonReplacementMatch(json[key])) {
        let index = json[key].replace(/\D/g, "");
        json[key] = stringList[index];
      }
    } else if (isArray(json[key])) {
      searchArray(json[key]);
    } else if (getType(json[key]) === "object" && json[key]) {
      restoreStringifiedJson(json[key], stringList);
    }
  });
  return json;
}
...
/**
 * create a new json parse callback
 * @param {Array} arr array for value storage
 */
function jsonParseCallback(arr) {
  this.push = arr.push;
  /**
   * callback for json parse to handle stringified json values
   * @param {string} key key for json parse
   * @param {*} value arbitrary value
   * @returns {string} value to send to json parse callback
   */
  this.callback = function (key, value) {
    if (isString(value)) {
      if (value.indexOf("{") !== -1) {
        try {
          JSON.parse(value);
          this.push(value);
          return "__json_value_" + jsonStringValues.length;
        } catch {
          return "!!! ERROR -- Unable to parse stringified json.";
        }
      }
    }
    return value;
