...
const helpers = {
  ...
  checkResourceTests: function (resources) {
    if (!Array.isArray(resources)) {
      throw new Error(
        "Expected resource tests to be an array got " + typeof resources + "."
      );
    }
  ...
  eachKey: function (obj, callback) {
    if (typeof obj !== "object" || Array.isArray(obj)) {
      ...
      if (argumentString.split(",").length > 1) {
        throw new Error(
          `eachKey callback function accepts only one argument got ` +
            JSON.stringify(argumentString.split(","))
        );
      }
    }
    // Run the callback
  ...
  // containsModule, expectedResourceAddress, getFoundResources, parseTestModuleOptions, composeName, isFunction
  ...
  tfVarCheck: function (tfvarObject) {
    let errList = [];
    helpers.eachKey(tfvarObject, (key) => {
      if (
        typeof tfvarObject[key] !== "string" &&
        typeof tfvarObject[key] !== "number" &&
        typeof tfvarObject[key] !== "boolean"
      ) {
        errList.push(
