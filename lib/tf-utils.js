...
const utils = function (options) {
  ...
  this.it = containsKeys(options, "overrideIt", true) ? options.overrideIt : it;
  this.describe = containsKeys(options, "overrideDescribe", true)
    ? options.overrideDescribe
    : describe;
  this.assert = containsKeys(options, "overrideAssert", true)
   ...
   * @param {string} address address to be tested
   */
  ...
  this.buildModuleTest = (moduleName, address, tfPlan, resources) => {
    ...
    let containModule = false; // object containsKeys module
    let containsKeysModuleData = containsKeysModule(moduleData, address); // Get module data if possible

    if (typeof containsKeysModuleData === "boolean") {
      containModule = containsKeysModuleData; // If containsKeys module data is boolean set to containsKeys module
    } else {
      // Otherwise set these from data
      containModule = containsKeysModuleData.containsKeysModule;
    ...
    if (containModule && resources.length > 0) {
      // For each resource
    ...
    let foundResources = [];
    if (containModule && moduleData !== undefined) {
   ...
   * @param {Array=} options.outputList list of output tests to run
  ...
  this.testModule = (options) => {
    ...
    if (buildOptions.isOutput) {
      moduleData = this.buildOutputTests(moduleName, tfData, testList);
    } else if (isApply) {
          ...
          this.describe(test.describe + " connection tests", () => {
            test.connectionTests.forEach((connectionTest) => {
              this.it(connectionTest.name, () => {
                return connectionTest.test()(connectionTest.address);
              });
            });
          });
        }
   ...
   * @param {Array<Object>} instanceValues Array of expected outputs
   * @returns {object} json test data
   */
  this.buildOutputTests = function (templateName, tfstate, outputValues) {
    let returnData = {
      describe: templateName + " Outputs",
      tests: [],
    };
    if (keys(tfstate.outputs).length === 0 && outputValues.length !== 0) {
      returnData.tests.push(
        deepEqualTest(`tfstate should have correct outputs`, [
          splat(outputValues, "name"),
          [],
          "Expected outputs",
        ])
      );
    ...
    outputValues.forEach((output) => {
      let existsInTfState = containsKeys(tfstate.outputs, output.name);
      let outputTest = {
        describe: output.name,
        tests: [
          notFalseTest(`Output ${output.name} should be in tfstate`, [
            existsInTfState,
            `Expected output ${output.name} to be in tfstate`,
          ]),
        ],
      };
      if (existsInTfState) {
        outputTest.tests.push(
          deepEqualTest(
            `Output ${output.name} should have the correct output value`,
            [
              output.value,
              tfstate.outputs[output.name].value,
              `Expected ${output.name} to have the correct value`,
            ]
          )
        );
      }
      returnData.tests.push(outputTest);
    });
    return returnData;
  };
        ...
        if (instanceData) {
          ...
          attributeList.forEach((attribute) => {
            ...
            if (
              ...
              typeof instanceData[attribute][0] === "object" &&
              Array.isArray(instanceData[attribute][0]) === false // And that type is object
            ) {
              ...
              if (isFunction(testValue)) {
                if (testValue.name === "connectionTest") {
                  connectionTests.push({
                    name: `${address}[${index}] connection tests`, // test name
                    test: testValue, // test value function
                    address: instanceData[attribute], // address used to evaluate function in run test step
                  });
                } else {
   ...
   * @returns An object containing each of the tests for each resource
   */
