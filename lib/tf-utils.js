...
const utils = function (options) {
  ...
  this.buildModuleTest = (moduleName, address, tfPlan, resources) => {
    ...
    let containsKeysModuleData = helpers.containsKeysModule(moduleData, address); // Get module data if possible
  ...
  this.testModule = (options) => {
    ...
    keyCheck(
      "options must be passed with key",
      options,
      ["tfData"]
    )
