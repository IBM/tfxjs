...
const utils = function (options) {
  ...
  this.assert = containsKeys(options, "overrideAssert")
    ...
    : assert;
  /**
   * Connection Test constructor. This constructor is supposed to take a callback function
   * that uses tfx.connect scoped outside of the callback. When invoked and provided an address
   * connectionTest()(address) it will execute the callback function. The second set of parentheses 
   * is needed here to prevent the callback from being executed with no address parameter
