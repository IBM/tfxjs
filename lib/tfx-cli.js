const { containsAny, flagValues, isEmpty, isString } = require("lazy-z");
const { planTfx, deyamilfy, applyTfx } = require("./extract");
const fs = require("fs");
const chalk = require("chalk");
const { convertTfVarsFromTags, restoreStringifiedJson } = require("./helpers");
const tfxInit = require("./tfx-init");
const { escapeStringifiedText } = require("./text-replacements");
...
const cli = function (child, spawn, ...commandArgs) {
  ...
  this.state = function () {
    commandArgs.shift();
    let stateFlagValues = flagValues(
      "state",
      this.verb.state,
      this.tags,
      ...commandArgs
    );
    let jsonStringValues = [];
    let stateJson = JSON.parse(
      this.readFileSync(stateFlagValues.in, "utf8"),
      (key, value) => {
        if (isString(value)) {
          if (value.indexOf("{") !== -1) {
            try {
              JSON.parse(value);
              jsonStringValues.push(value);
    ...
    restoreStringifiedJson(stateJson, jsonStringValues);
    this.applyTfx(
      stateJson,
      stateFlagValues.in,
      this.writeFileCallBack(stateFlagValues.out)
    );
  };
