...
To test a .js file:
`) + chalk.yellow(`  $ tfx <file_path>

`) + chalk.white(`To create tests from a terraform plan:
`) + chalk.yellow(`  $ tfx plan --in <terraform file path> --out <filepath> --type <tfx or yaml>

`) + chalk.white(`Additional flags are also available:
`) + chalk.yellow(`  -v | --tf-var
...
`) + chalk.yellow(`  $ tfx decode <yaml file path> --out <filepath>
