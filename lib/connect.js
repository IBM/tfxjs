...
const connect = function (connectionPackages) {
  /**
   * Uses GNU netcat version 0.7.1 to test UDP connection, and runs a chai assertion when finished.
   * Both successful and unsuccessful connections can be created. If the connection is not refused
   * within the timeout window, it will be assumed as successful.
   ...
   */
  this.sshTest = function (host, username, privateKey, doesNotConnect) {
    ...
    return connectionPackages.ssh
      ...
      .then(() => {
        assertionData = {
          name: `expected ssh test at ${host} to ${
            doesNotConnect ? "not " : ""
          }connect`,
  ...
  this.pingTest = function (host, doesNotConnect) {
    return connectionPackages.ping.promise.probe(host).then((res) => {
      assert.deepEqual(
        res.alive,
        !doesNotConnect,
        `expected ping at ${host} ${doesNotConnect ? "not " : ""}to connect`
      );
    });
  };
  /**
