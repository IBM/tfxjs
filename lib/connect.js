...
const connect = function (connectionPackages) {
  ...
  this.sshTest = function (
    ...
    return connectionPackages.ssh
      ...
      .then(() => {
        assertionData = {
          name: `expected ssh test at ${host} to ${doesNotConnect ? "not " : ""}connect`,
  ...
  this.pingTest = function (host, doesNotConnect) {
    return connectionPackages.ping.promise.probe(host).then((res) => {
      assert.deepEqual(res.alive, !doesNotConnect, `expected ping at ${host} ${doesNotConnect ? "not " : ""}to connect`);
    });
  };
  /**
