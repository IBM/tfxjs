...
const extract = {
   ...
   */
  planResourceTest: function (resource, outputType, includeNull, spaces, shallow) {
   ...
   */
  moduleTest: function (tfModule, outputType, includeNull, isSubModule, shallow) {
    ...
    if (containsKeys(tfModule, "resources"))
      tfModule.resources.forEach((resource) => {
        resources.push(
          extract.planResourceTest(resource, outputType, includeNull, 0, shallow)
        );
      });
    ...
    if (containsKeys(tfModule, "child_modules")) {
      ...
      tfModule.child_modules.forEach((child) => {
        ...
        if (outputType === "tfx") {
          let extactedData = extract.moduleTest(child, outputType, includeNull, isSubModule, shallow);
          // If tfx add string
          outputData += "\n" + extactedData;
        } else {
          // If yaml add as key
   ...
   */
  planTfx: function (templateName, templatePath, type, tfvars, exec, shallow, callback) {
    let cli = new tfCli(templatePath, exec, true);
    return cli.plan(
      tfvars,
      (planData) => {
        let planModules = extract.moduleTest(planData, type, false, false, shallow);
        if (type === "yaml") {
          planModules =
            YAML.stringify({
              tfxPlan: {
