...
const extract = {
  ...
  moduleTest: function (tfModule, outputType, includeNull, isSubModule) {
    ...
    if(outputType !== "yaml" && outputData.match(/tfx.module\(\s+\".+\",\s+\'.+\',\s+\{\}\s+\);/g)) {
      outputData = "";
    }
    ...
    await cli.plan(
      ...
      (planData) => {
        ...
        callback(
          testTemplate
            .clone()
            .fill(templatePath, prettyJSON(tfvars), templateName,  matchSpace(1, planModules))
        );
      },
      { cleanup: true }
    );
  ...
  deyamilfy: function (yamlFileData) {
    ...
    eachKey(jsonData, (key) => {
      ...
      // Prevent from returning modules with none resources which causes tfx to throw
      if(resources.length > 0) {
        newModule.set(
          "$VALUES", matchSpace(1, resources.join("\n"))
        );
        strData += "\n" + newModule.str;
      }
    });
