...
const { prettyJSON, containsCheck, containsKeys, keys } = require("./utils");
...
const extract = {
  ...
  planResourceTest: function (resource, outputType, includeNull, spaces) {
    ...
    let resourceName = capitalizeWords(resource.name.replace(/(_|-)/g, " ")); // Resource name
    ...
    if (containsKeys(tfModule, "child_modules")) {
      ...
      tfModule.child_modules.forEach((child) => {
        ...
        if (outputType === "tfx") {
          ...
          // If yaml add as key
          eachKey(childData, (key) => {
            outputData[key] = childData[key];
          });
        }
      });
    }
  ...
  deyamilfy: function (yamlFileData) {
    ...
    eachKey(jsonData, (key) => {
      ...
      jsonData[key].resources.forEach(resource => {
        ...
        resources.push(
          newResource.fill(resourceName, resourceAddress, matchSpace(1,prettyJSON(valueObject)))
        )
      });
      newModule.set("$VALUES", resources.length === 0 ? "  {}" : matchSpace(1,resources.join("\n")))
