...
const {
  ...
  tfxModuleRegExp,
  notEscapedQuotationMarks,
  extraWhitespaceRegExp,
  newLineBeforeNextBlockRegExp,
  quotesAroundStringsAndEscapedSlashes,
  escapedQuotesRegExp,
...
const extract = {
  ...
  planResourceTest: function (
    ...
    if (deepEqual(resource, { timeouts: null })) {
      return "";
    } else {
      ...
      let resourceName = capitalizeWords(
        indexName.replace(indexNameRegExp, " ")
      ); // Resource name
      let resourceAddress =
        (resource.mode === "data" ? "data." : "") +
        resource.address.replace(resourceAddressRegExp, "");
    ...
    return isSubModule || outputType === "tfx"
      ...
      : "\n" +
          YAML.stringify(outputData)
            .replace(/"-"/g, "@@@")
            .replace(/:\s?"/g, "?????") //replace unescaped quotation mark with 5 question marks to not use look.behind
            .replace(/\?\?\?\?\?/g, ": ") //return colon and space
            .replace(/-\s?"/g, "?????")
            .replace(/\?\?\?\?\?/g, "- ") //return dash and space
            .replace(/"(?=\n)/g, "") //replace quotation marks followed by new line
            .replace(/\\(?=")/g, "") //replace slashes followed by quotation mark
            .replace(/@@@/g, '"-"');
  },
