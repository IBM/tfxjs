  ...
  deyamilfy: function (yamlFileData, tfvars) {
    ...
    eachKey(jsonData, (key) => {
      if (key !== "tfxPlan") {
        ...
        let tfxTemplateData = jsonData[key];
        jsTestTemplate.set("$TEMPLATE_PATH", tfxTemplateData["template-path"] || "<template path>")
        jsTestTemplate.set("$TEMPLATE_NAME", tfxTemplateData["template-name"] || "<template name>")
        if(containsKeys(tfxTemplateData, "tfvars")) {
          eachKey(tfxTemplateData.tfvars, (key) => {
            if(!containsKeys(overrideTfvars, key)) {
              ...
              console.log(overrideTfvars)
            }
          })
        }
        jsTestTemplate.set("$TF_VARS", prettyJSON(overrideTfvars))
      }
    });
    strData = strData
      .replace(/\n\s*\n+/g, "\n")
