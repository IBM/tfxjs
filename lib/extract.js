...
<<<<<<< HEAD
    ...
    let resourceAddress = (resource.mode === "data" ? "data." : "") + resource.address
      .replace(/.+\.(?=.+\..+)/g, "")
...
=======
    const indexNameRegExp = new RegexButWithWords()
      .group((exp) => {
        exp.literal("_").or().literal("-");
      })
      .done("g");
    const resourceAddressRegExp = new RegexButWithWords()
      .any()
      .oneOrMore()
      .literal(".")
      .look.ahead((exp) => {
      ...
      .done("g");
    const notEscapedQuotationMarks = new RegexButWithWords()
      .literal('"')
      .done("g");
    let resourceName = capitalizeWords(indexName.replace(indexNameRegExp, " ")); // Resource name
    let resourceAddress =
      (resource.mode === "data" ? "data." : "") +
      resource.address
        .replace(resourceAddressRegExp, "")
        .replace(notEscapedQuotationMarks, '\\"'); // Resource Address
    // List of values
    let values = includeNull
      ? resource.values // if include null, return values
...
=======
    const tfxModuleRegExp = new RegexButWithWords()
      .literal("tfx.module(")
      .whitespace()
      .oneOrMore()
      .literal('"')
...
=======
    const extraWhitespaceRegExp = new RegexButWithWords()
      .newline()
      .whitespace()
      .anyNumber()
      .newline()
      .oneOrMore()
      .done("g");
    const newLineBeforeNextBlockRegExp = new RegexButWithWords()
      .literal(";")
      .newline()
      .look.ahead((exp) => {
        exp.literal("tfx");
    ...
    // If is submodule or tfx, return raw output, otherwise yaml stringify sata
    const quotesAroundStringsAndEscapedSlashes = new RegexButWithWords().look
      .behind((exp) => {
        exp.whitespace();
      })
      .literal('"')
      ...
      .done("g");
    const escapedQuotesRegExp = new RegexButWithWords()
      .literal('\\\\\\"')
      .done("g");
>>>>>>> intern-tfxjs/master
...
=======
            .replace(quotesAroundStringsAndEscapedSlashes, "") // remove quotes around string type variables and also escaped slashes
            .replace(escapedQuotesRegExp, '"'); // replace escaped quotes with single quotes
  },
...
<<<<<<< HEAD
   * @param {extractCallback} callback Callback function
   */
  planTfx: function (templateName, templatePath, type, tfvars, exec, callback) {
    let cli = new tfCli(templatePath, exec, true);
    return cli.plan(
      tfvars,
      (planData) => {
...
=======
        const spacesBeforeOpenBraces = new RegexButWithWords()
          .whitespace()
          .whitespace()
          .look.ahead((exp) => {
            exp.literal("{");
        ...
        jsTestTemplate.set(
          "$TF_VARS",
          // Match spaces and replace leading spaces before tfvar opening brace with spaces for formatting
          matchSpace(1, prettyJSON(overrideTfvars)).replace(spacesBeforeOpenBraces, "")
        );
      }
    });
    const extraNewlinesRegExp = new RegexButWithWords()
      .newline()
      .whitespace()
      .anyNumber()
      .newline()
      .oneOrMore()
      .done("g");
    const endingNewlinesRegExp = new RegexButWithWords()
      .literal(";")
      .newline()
      .look.ahead((exp) => {
        exp.literal("tfx");
