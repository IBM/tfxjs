...
const moduleOutputTemplate = new textTemplate(
  `tfx.output($OUTPUT_NAME,\n$VALUE)`
);
...
function applyResourceTest(resource) {
  ...
  address.set("$RESOURCE_ADDRESS", formatResourceAddress(resource, true));
...
=======
      .replace(/:\s(?=[^"\{\[}])(?!(null|true|false))/g, ": \"") // replace unclosed string values taken away with JSON.parse
>>>>>>> 0a41793 (merge)
      .replace(/:\{/g, ": {") + ");";
...
tfx.apply("tfxjs generated tests", () => {\n  `;
  if (tfState.outputs && keys(tfState.outputs).length > 0) {
    applyText += `tfx.outputs("template outputs",\n`;
    let allOutputs = "";
    eachKey(tfState.outputs, (output) => {
      allOutputs += `tfx.output("${output}", ${prettyJSON(
        tfState.outputs[output].value
      )}),\n`;
    });
    applyText += indent(allOutputs.replace(/,\n(?=$)/i, ""), 4);
    applyText += "\n  );\n  ";
  }
