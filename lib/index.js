...
`);
  ...
  this.module = function (moduleName, moduleAddress, resources) {
    ...
    this.tfutils.testModule(moduleName, moduleAddress, this.tfplan, resources);
  };
   ...
   */
   this.state = function (moduleName, resources) {
    if(!this.state) {
      throw new Error("`tfx.apply` needs to be successfully completed before running `tfx.module`.")
    }
    this.tfutils.testModule(moduleName, "", this.tfstate, resources, true);
  };
};
