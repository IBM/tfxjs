const tfutils = require("./tf-utils.js"); // Terraform utils
...
const { checkResourceTests } = require("./helpers.js");
const { testHead } = require("./builders.js");
const { eachKey, containsCheck, typeCheck, containsKeys } = require("lazy-z");
...
const tfx = function (templatePath, tfvars, options) {
  this.tfutils = new tfutils(options); // Initialize tf utils with local templatepath and the test directory
  ...
  this.connectionTest = this.tfutils.connectionTest; // add connectionTest
  this.connect = new builders.connect(connect, {
    ssh: containsKeys(options, "overrideSSH", true) ? options.overrideSSH : ssh,
    ping: containsKeys(options, "overridePing", true)
      ? options.overridePing
      : ping,
    exec: containsKeys(options, "overrideExec", true)
  ...
  this.cli = new tfCli(
    ...
    containsKeys(options, "overrideExec", true) ? options.overrideExec : exec,
    // If exec is overridden or quiet is passed, disable logs
    containsKeys(options, "overrideExec", true) ||
    containsKeys(options, "quiet", true)
      ? false
      : this.enableLogs
  );
  ...
  /**
   * run code to get tfstate data from filepath without apply
   */
  this.runTestsFromTfState = async function () {
    await this.cli.apply(
      {},
      (data) => {
        this.tfstate = data;
      },
  ...
  this.tfAction = function (
    ...
    containsCheck(
      `tfAction currently only accepts ${JSON.stringify(actionTypes)},`,
      actionTypes,
      actionName
    );
    typeCheck(
      `tfx.${actionName} expected callback to be a`,
      "Function",
      callback
    );
    typeCheck(
      `tfx.${actionName} expected beforeFn to be a`,
      "Function",
      beforeFn
    );
  ...
  /**
   * Run tests for a module outputs
   * @param {string} moduleName decorative string for module name
   * @param {...{outputName: string, instances: Array<Object>}} resources Object of resources from the module to check
   */
  this.outputs = function (moduleName, ...resources) {
    // Throw an error if no tfstate
    if (!this.tfstate) {
      throw new Error(
        chalk.red(
