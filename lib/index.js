...
const tfx = function (templatePath, tfvars, options) {
  ...
  this.module = function (moduleName, moduleAddress, ...resources) {
    ...
    if (!this.tfplan) {
      throw new Error(
        chalk.red("`tfx.plan` needs to be successfully completed before running `tfx.module`.")
      );
    }
  ...
  this.state = function (moduleName, ...resources) {
    ...
    if (!this.tfstate) {
      throw new Error(
        chalk.red("`tfx.apply` needs to be successfully completed before running `tfx.state`.")
      );
    }
