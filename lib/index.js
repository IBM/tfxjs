...
const tfx = function (templatePath, tfvars, options) {
  ...
  this.tfutils = new tfutils(options);
  ...
  try {
    this.exec = options.overrideExec;
    this.cli = new tfCli(templatePath, this.exec, false);
  } catch {
    this.exec = exec;
    this.cli = new tfCli(templatePath, this.exec, options?.quiet || true);
  }
