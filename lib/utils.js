...
const utils = function (scriptPath, templatePath, apiKeyVariableName, options) {
  ...
  this.buildResourceTest = function (
    ...
    try {
      ...
      moduleData.resources.forEach((resource) => {
        let addressToCheck = moduleData.address === "root_module" ? address : `${moduleData.address}.${address}`
  ...
  this.buildModuleTest = (moduleName, address, tfPlan, resources) => {
    ...
    try {
      ...
      if (
        parentAddress === "" &&
        !helpers.keysContains(moduleData, "resources")
      ) {
        throw new Error(
    ...
    if (moduleData.address === address) {
      ...
      try {
        ...
        let searchData = helpers.childArraySearch(parentAddress, childArray)
        containsModule = searchData.containsModule
        moduleData = searchData.moduleData
      } catch {
        // Catch set to false
      ...
      resources.forEach((resource) => {
        ...
        let expResourceAddress =
          parentAddress === ""
            ? resource.address
            : parentAddress
            ? `${parentAddress}.${resource.address}`
    ...
    if (containsModule && moduleData !== undefined) {
      if (moduleData.resources) {
        ...
        foundResources = helpers.getFoundResources(moduleData.resources, address, expectedResources)
