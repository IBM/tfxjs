...
const utils = function () {
  ...
  this.flagTest = (command, aliases, ...commandArgs) => {
    ...
    if(this.keys(aliases).length !== this.keys(flagMap).length) {
      let errString = `Missing flags from command ${command}`
   ...
   * @param {string} action.requiredFlags Flags required by the action
