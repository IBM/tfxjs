...
const utils = function (scriptPath, templatePath, apiKeyVariableName, options) {
   ...
   */
  this.testModule = (moduleName, address, tfplan, resources, isApply) => {
    // Build module level test data
    let moduleData;
    if (isApply) {
    } else {
      moduleData = this.buildModuleTest(moduleName, address, tfplan, resources);
    }
   ...
   * @returns JSON test data
   */
  this.buildInstanceTest = (resourceName, address, tfstate, instanceValues) => {
    let resourceData = false;
    // For each resource in the state

    tfstate.resources.forEach((resource) => {
      let fullName = this.composeName(resource);
      if (fullName == address) {
        resourceData = resource;
      }
    });
  ...
  this.buildStateTest = (moduleName, tfstate, instanceTests) => {
    ...
    instanceTests.forEach((test) => {
      ...
      test.instances.forEach(instance => {
        testObj[instance.index_key] = instance
        delete testObj[instance.index_key].index_key
      })
      // build tests and add to tests
      let testData = this.buildInstanceTest(
        test.name,
        test.address,
        tfstate,
        testObj
      );
      instanceObject.tests.push(testData);
    });
