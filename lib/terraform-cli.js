...
const terraformCli = function (directory, exec, enableLogs) {
  ...
  this.plan = function (tfvars, callback, options) {
    ...
    return this.init(tfvars) // Initialize terraform with tfvars
      .then((data) => {
        /*
        if (contains(data.stderr, "Error in")) {
          throw {message: "Error in ../directory/main.tf"};
        }
        */
        if (contains(data.stdout, "The directory has no Terraform configuration files.")) {
          throw {message: "Error: Terraform initialized in empty directory " + this.directory + "\n\nInsert Terraform configuration files into the directory and try again\n"};
        }
        if (contains(data.stdout, "Enter a value:")) {
          throw {message: "Error: A required variable is not provided as part of a terraform plan"};
        }
  ...
  this.tfErr = (err) => {
    this.print(
      chalk.red(`${err.stderr.replace(/\s(?=\S+\.tf)/g, ` ${this.directory}/`)}`)
    );
  };
};
