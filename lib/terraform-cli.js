const { eachKey, keysContains } = require("./helpers");
...
const terraformCli = function (directory, exec, enableLogs) {
  ...
  this.cdAndExec = function (command) {
    return this.execPromise(`cd ${directory}\n${command}`);
  };
  ...
  this.setTfVarString = function (tfvars) {
    if (typeof tfvars !== "object" || Array.isArray(tfvars))
      throw new Error(
        "setTfVarString expected param of type object got " +
          (Array.isArray(tfvars) ? "Array" : typeof tfvars)
      );
  ...
  this.plan = function (tfvars, callback, options) {
    ...
    return this.init(tfvars) // Initialize terraform with tfvars
      ...
      .then((data) => {
        ...
        if (!keysContains(options, "no_output"))
   ...
   */
  this.tfErr = function (err) {
    throw new Error(
      `${err.stderr.replace(/\s(?=\S+\.tf)/g, ` ${directory}/`)}`
    );
  };
};
