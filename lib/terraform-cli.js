...
const terraformCli = function (directory, exec, enableLogs) {
  ...
  this.clone = function (clonePath) {
    return this.execPromise(
      `mkdir ${clonePath} && rsync -av --progress --exclude='*.tfvars' --exclude='*.tfstate' ${this.directory} ${clonePath} -q` // --exclude '*.tfvars' -q`
    ).catch(this.tfErr);
  };
  ...
  this.setTfVarString = function (tfvars) {
    typeCheck("setTfVarString expected param of type", "object", tfvars);
    ...
    return this.init(tfvars) // Initialize terraform with tfvars
      .then((data) => {
        ...
        if (options?.no_output) {
          ...
          const hypyenRegExp = new RegexButWithWords()
            .whitespace()
            .literal("-")
            .any()
            .oneOrMore()
            .done("g");
          command = command.replace(hypyenRegExp, "");
        }
        return this.cdAndExec(
   ...
   */
  this.tfErr = (err) => {
    const errRegExp = new RegexButWithWords()
      .whitespace()
      .look.ahead((exp) => {
        exp.notWhitespace().oneOrMore().literal(".tf");
      })
      .done("g");
    let errorText = `${err.stderr.replace(errRegExp, ` ${this.directory}/`)}\n`;
