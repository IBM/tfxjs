const { typeCheck, contains, hclEncode, containsKeys } = require("lazy-z");
const chalk = require("chalk");
const { inOutExp, fileNameRegExp } = require("./constants");
const fs = require("fs");
/**
...
const terraformCli = function (directory, exec, enableLogs) {
  ...
  this.plan = function (tfvars, callback, options) {
    ...
    return this.init(tfvars) // Initialize terraform with tfvars
      ...
      .then((data) => {
        ...
        if (!containsKeys(options, "no_output", true))
   ...
   * @param {string=} tfStateFilePath use tfstate instead of running apply
   * @returns {Promise} Apply promise
   */
  this.apply = function (tfvars, callback, destroyAndCleanup, tfStateFilePath) {
    let jsonData;
    if (tfStateFilePath) {
      return this.cdAndExec("cat " + tfStateFilePath)
        .then((data) => {
   ...
   */
  this.tfErr = (err) => {
    let errorText = err?.stderr
      ? `${err.stderr.replace(fileNameRegExp, ` ${this.directory}/`)}\n`
      : err + "\n";
