...
const builders = {
  ...
  eval: function (appendMessage, evaluationFunction) {
    ...
    } else if (!(evaluationFunction instanceof Function)) {
      throw new Error("tfx.expect expected evaluationFunction to be a function");
    }
   ...
   */
  check: function (address, ...instances) {
    if (typeof address !== "string") {
      throw new Error(
        `tfx.check expects address to be a string got ${typeof address}`
      );
    }
    if (instances.length === 0) {
      throw new Error(
        `tfx.check expects at least one instance got ${instances.length}`
      );
    }
    ...
    if (!allObjects) {
      throw new Error(
        `tfx.check expected all instances to be of type Object got ${JSON.stringify(
          instanceTypes
        )}`
      );
    }
