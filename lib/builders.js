...
`);
  ...
  textTemplate: function (str) {
    ...
    this.str = str;
    this.templateArgs = [];
    const dollarSignRegExp = new RegexButWithWords()
      .any()
      .anyNumber()
      .look.ahead((exp) => {
        exp.literal("$");
      });
    const trailingCharactersRegExp = new RegexButWithWords().look
      .behind((exp) => {
      ...
      .done("g")
      .done("g");
    const templateRegExp = new RegexButWithWords()
      .literal("$")
      .set("A-Z_")
      .done("g");
    // Split at chatacter before dollar sign
    str.split(dollarSignRegExp).forEach((substr) => {
      // remove trailing characters
      let capsOnly = substr.replace(trailingCharactersRegExp, "");
      // Push if matches valid template value
      if (capsOnly.match(templateRegExp)) this.templateArgs.push(capsOnly);
    });
