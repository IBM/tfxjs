...
function eachKeyTest(address, values, data, type, attribute, index) {
  ...
  if (Array.isArray(values)) {
    testList.push(
      deepEqualTest(
        `Expected resource ${address}[${index}] to have correct value for ${attribute}[${index}]`,
        [
          values,
          [data],
          `Expected ${address} to have correct value for ${address}[${index}].`,
        ]
      )
    );
  } else
...
function textTemplate(str) {
   ...
   * @returns {textTemplate} Text templateobject
   */
  this.clone = function () {
