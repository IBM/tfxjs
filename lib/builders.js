...
`);
  ...
  connect : {
    ...
    tcp: {
      doesConnect: (host, port, connectPackage) => {
        return new connect({ exec: connectPackage }).tcpTest(host, port);
      },
      doesNotConnect: (host, port, connectPackage) => {
        return new connect({ exec: connectPackage }).tcpTest(host, port, true);
      },
    },
  },
};
